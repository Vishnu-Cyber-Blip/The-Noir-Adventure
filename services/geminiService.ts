...
5. **Choices**: Provide exactly 4 distinct, actionable choices for the player to advance the story. One should usually be risky, one investigative, one cautious.
...
Structure:
{
  ...
  "choices": ["Inspect the body", "Check the window", "Call headquarters", "Leave quietly"],
  "stats": { ... },
  "inventory": ["Item1"],
  "characters": [{ "name": "Detective Joe", "description": "Grizzled partner.", "status": "Alive" }],
  "new_achievements": [{ "id": "first_blood", "title": "First Blood", "description": "You witnessed a murder." }] (Optional)
}
`;
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    choices: { 
      ...
      description: "4 distinct choices for the player." 
    },
    stats: {
      type: Type.OBJECT,
      properties: {
  ...
  required: ["narrative", "choices", "stats", "inventory", "characters"],
};
...
export const startNewStory = async (premise: string): Promise<StoryResponse> => {
  ...
  try {
    const prompt = `START GAME. Premise: "${premise}". Initialize stats (100/100/0). Give 1 starting item. Initialize character list. Provide 4 starting choices.`;
    ...
    return {
        ...
        choices: ["Try again", "Check connection"],
