...
function App() {
  ...
  const [activeModal, setActiveModal] = useState<'inventory' | 'characters' | 'storyline' | 'news' | 'settings' | null>(null);
  ...
  const handleUserAction = useCallback(async (actionText: string) => {
    ...
    setCurrentChoices([]); // Clear choices while thinking
    setIsProcessing(true);
    ...
    try {
      ...
      setCurrentChoices(["Retry Action"]);
    } finally {
      setIsProcessing(false);
    }
  ...
  const renderGameScreen = () => (
    <div className={`h-screen w-full relative z-10 ${shake ? 'shake-anim' : ''} fade-enter-active flex flex-col overflow-hidden`}>
      ...
      {activeModal === 'settings' && (
          <ModalOverlay title="Game Settings" onClose={() => setActiveModal(null)}>
              <div className="space-y-6 p-2">
                  <div>
                      <h4 className="text-zinc-500 font-retro uppercase text-xs mb-3">Text Size</h4>
                      <div className="flex gap-2">
                          {(['sm', 'base', 'lg', 'xl'] as const).map(size => (
                              <button
                                  key={size}
                                  onClick={() => setSettings(s => ({ ...s, textSize: size }))}
                                  className={`flex-1 py-2 border font-serif text-${size} ${
                   ...
                   <div>
                      <h4 className="text-zinc-500 font-retro uppercase text-xs mb-3">Typing Speed</h4>
                      <div className="flex gap-2 items-center">
                          <span className="text-xs text-zinc-600">Slow</span>
                           <input 
                              ...
                              onChange={(e) => setSettings(s => ({ ...s, typingSpeed: Number(e.target.value) }))}
      ...
      <div className="flex-1 overflow-y-auto px-6 max-w-3xl mx-auto w-full scrollbar-hide">
        <div className="pt-4 pb-4">
            ...
            <div ref={bottomRef} className="h-40" /> {/* Larger spacer for bottom choice bar */}
        </div>
      </div>
